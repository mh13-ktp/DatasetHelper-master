...
public class CameraActivity extends AppCompatActivity implements View.OnClickListener, OptionView.Callback {
   ...
   public static String videoFilePath;
    ...
    private void pictureFromVideo(File movieFile){
        ...
        System.out.println("-i " + videoFilePath + " -r 30/1 "+ initialFolderPath +"/out%03d.bmp");

        int returnCode = FFmpeg.execute("-i " + videoFilePath + " -r 30/1 "+ initialFolderPath +"/out%03d.bmp");
        ...
        randomOrientationFlip();
    }

    private void randomOrientationFlip() {
        ...
        for (int i = 0; i < files.length; i++) {
            if (!files[i].getName().endsWith(".mp4")) {
                ...
                Bitmap bitmapSrc = BitmapFactory.decodeFile(files[i].getPath());

                int orientation = random.nextInt(4 - 0);
                switch (orientation) {
                    case 0:
                        //this should just be standard normal orientation?
                        break;
                    case 1: // flip vertical
                        createFlippedBitmap(bitmapSrc, imageFilePath,true,false);
                        System.out.println("Case 1");
                        break;
                    case 2: //flip horizontal
                        createFlippedBitmap(bitmapSrc, imageFilePath, false,true);
                        System.out.println("Case 2");
                        break;
                    case 3: //flip horizontal and vertical
                        createFlippedBitmap(bitmapSrc, imageFilePath ,true,true);
    ...
    public static void createFlippedBitmap(Bitmap source, String imagePath, boolean xFlip, boolean yFlip) {
        Matrix matrix = new Matrix();
        File file = new File(imagePath);
        Bitmap mutableSorceCopy = BitmapFactory.decodeFile(file.getAbsolutePath());
        mutableSorceCopy = source.copy(Bitmap.Config.ARGB_8888,true); //no location
        matrix.postScale(xFlip ? -1 : 1, yFlip ? -1 : 1, mutableSorceCopy.getWidth() / 2f, mutableSorceCopy.getHeight() / 2f);
        Bitmap.createBitmap(mutableSorceCopy, 0, 0, mutableSorceCopy.getWidth(), mutableSorceCopy.getHeight(), matrix, true);
        try{
            FileOutputStream fos = new FileOutputStream("/sdcard/Frames/Rotated");
            mutableSorceCopy.compress(Bitmap.CompressFormat.JPEG,100,fos);
            fos.flush();
            fos.close();
        } catch (Exception e) {
            e.printStackTrace();
